...
def run():
  ...
  #T/N stage JSON 값 오류..? T2 만 output으로 도출되어야 하는데, T2.0으로 도출될 가능성이 있어 보임
def _normalize_stage(value, prefix: str, lo: int, hi: int) -> str:
    ...
    # 4. Prognosis → float
  #RFS JSON이 NaN/Infinity로 저장되지 않게 수정
...
def (ct_path, pet_path, ehr, segmentation_array) -> tuple[str, str]:
    """
...
# 이부분도 수정함.. (T2.0으로 OUTPUT이 도출되는 문제)
    def _predict_raw(pkg, df):
        model = pkg["model"]
    ...
    raw_t = _predict_raw(t_pkg, df.copy())
    raw_n = _predict_raw(n_pkg, df.copy())
